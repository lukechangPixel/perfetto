...
bool QueryPlanBuilder::TrySortedConstraint(
  ...
  // Special case for id equality: we know for a fact that we will always get at
  // most one row in that case.
  if (ct.Is<Id>() && op.Is<Eq>()) {
  ...
  switch (rc.index()) {
    ...
    case base::variant_index<RowCountModifier, Div2RowCount>():
      plan_.params.estimated_row_count =
          std::max(1u, plan_.params.estimated_row_count / 2);
      break;
    ...
    case base::variant_index<RowCountModifier, LimitOffsetRowCount>(): {
      ...
      // the row count.
      plan_.params.estimated_row_count = plan_.params.estimated_row_count;
      break;
    }
    default:
